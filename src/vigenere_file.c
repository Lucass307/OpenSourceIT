...
/* ne rencontrons pas le marqueur de fin de fichier. */ 
    for(i = 0, j = 0; (texte = fgetc(input)) != EOF; ++i) {
        ...
        if(texte >= 'A' && texte <= 'Z') {
            /* Nous récupérons son décalage par rapport à 'A' de sorte que 'A' donne 0, 'B' donne 1, et ainsi de suite. */
            value = texte - 'A';
            /* Nous mettons le résultat à 'A' pour appliquer le décalage ensuite et conserver la connaissance d'une majuscule. */
            message = 'A';
        ...
        /* En cas d'un caractère non alphabétique, nous l'imprimons sans appliquer de décalage. */
            fputc(texte, output);
            continue;
        }
